...
describe('ABI 3 command encoding', () => {
  ...
  it('encodes paragraph underlines, Choice and Radio options without widening ABI3', () => {
    ...
    const { bytes, record } = packed([
      ...
      { type: 'form.create', pageId: 'page-1', fieldId: 'radio-1', name: 'Color',
        fieldType: 'radio', bounds, options: ['红色', '蓝色'] },
    ]);
    expect(bytes.byteLength).toBe(3 * EDIT_COMMAND_STRIDE);
    ...
    expect(record(2).string(3)).toBe('radio');
    expect(record(2).fields[10]).toBe(0);
    expect(record(2).ids).toEqual(['红色', '蓝色']);
  });
