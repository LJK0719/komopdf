...
export type ExportSettings = Pick<SaveRequest, 'protection' | 'password' | 'optimize'>;
...
export function ExportPanel({ disabled, encrypted, signed, onExport }: {
  ...
  return <section className="text-edit-panel" aria-label="PDF export options">
    <details><summary>Protection & export</summary>
      ...
      {signed && (optimize || protection !== 'preserve') && <p role="alert">Changing protection or optimizing this PDF will invalidate its existing digital signatures. Save to a different file to retain the signed original.</p>}
